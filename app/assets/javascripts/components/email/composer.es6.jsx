...
import SaveDraft from '../../mutations/mailbox/saveDraft.es6';
import DiscardDraft from '../../mutations/mailbox/discardDraft.es6';
...
class Composer extends Component {
  ...
  saveDraft() {
    const onFailure = transaction => this.onFailure(transaction);
    const onSuccess = response => this.onSuccess(response);
    Relay.Store.commitUpdate(new SaveDraft({
      conversationId: this.props.conversation.id,
    }), { onFailure, onSuccess });
  }
  ...
  discardDraft() {
    const onFailure = transaction => this.onFailure(transaction);
    const onSuccess = response => this.onSuccess(response);
    Relay.Store.commitUpdate(new DiscardDraft({
      conversationId: this.props.conversation.id,
    }), { onFailure, onSuccess });
  }
